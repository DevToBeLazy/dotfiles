  ...
  return Widget.Box({
    ...
    setup: (self) => self
      ...
      .on("key-press-event", (widget, event) => { // Typing
        ...
        if (checkKeybind(event, userOptions.keybinds.overview.altMoveLeft)) { entry.set_position(Math.max(entry.get_position() - 1, 0)); }
        else if (checkKeybind(event, userOptions.keybinds.overview.altMoveRight)) { entry.set_position(Math.min(entry.get_position() + 1, entry.get_text().length)); }
