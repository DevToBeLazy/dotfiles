...
const screen = await bash`ls -w1 /sys/class/backlight | tail -1`;
const touchbar = await bash`ls -w1 /sys/class/backlight | head -1`;
...
class Brightness extends Service {
  ...
  set screen(percent) {
    ...
    sh(
      `brightnessctl -d ${touchbar} set ${Math.floor(percent * 100) > 25 ? Math.floor(percent * 100) : 25}% -q`,
    );
  }
